...
class VehicleLog(Document):
	...
	def on_submit(self):
		frappe.db.set_value("Vehicle", self.vehicle_item, "last_odometer", self.odometer)

	def on_cancel(self):
		distance_travelled = self.odometer - self.last_odometer
		if(distance_travelled > 0):
			updated_odometer_value = int(frappe.db.get_value("Vehicle", self.vehicle_item, "last_odometer")) - distance_travelled
			frappe.db.set_value("Vehicle", self.vehicle_item, "last_odometer", updated_odometer_value)
